...
module flt_temp
  ...
  (
   ...
   output logic                      fh_temp_tvalid,
   output logic [31:0]               fh_temp_tdata
   );
  ...
  assign fh_temp_tvalid = fused_tvalid;
  assign fh_temp_tdata  = fused_tdata;

  assign fused_a_tvalid = temperature_valid; // mult_tvalid; // result_valid;
  assign fused_a_tdata  = temperature.raw; // mult_tdata; // result_data.raw;
  assign fused_b_tvalid = temperature_valid; // mult_tvalid; // result_valid;
  assign fused_b_tdata  = nine_fifths;
  assign fused_c_tvalid = temperature_valid; // mult_tvalid; // result_valid;
  ...
  always @(posedge clk) begin
    ...
    if (fused_tvalid) begin
      temperature.fp       <= fused_tdata;
      f_valid              <= '1;
    end
  ...
  u_xpm_fifo_sync
    (
     .sleep                  ('0),
     .rst                    (!rst_n), // flush the window on reset (active-high
